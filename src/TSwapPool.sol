...
contract TSwapPool is ERC20 {
    ...
    // q if its empty how does it warm up
    function deposit(
        uint256 wethToDeposit,
        uint256 minimumLiquidityTokensToMint, //* LP tokens —> if empty, we can pick 100% (100% == 17 tokens)
        uint256 maximumPoolTokensToDeposit,
        //* @audit-high deadline not used
        // @note if someone sets a deadline let's say next block , they could still deposit
        ...
        returns (uint256 liquidityTokensToMint)
    {
        ...
        //@audit-info MINIMUM_WETH_LIQUIDITY is a constant ie not required to be emitted
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            // if we calculate too many pool tokens to deposit , we revert
            // deposit weth 10
        ...
        returns (uint256 outputAmount)
    {
        ...
        //@audit-info magic numbers - 0.03% fees
        ...
        returns (uint256 inputAmount)
    {
        ...
        // 997 / 10000 
        // 91.3% fee
        // @audit -HIGH : users are gonna be charged too much
        // LIKELIHOOD :HIGH - swapExactOutput is the one of the main swapping function
    ...
    function swapExactInput(
        IERC20 inputToken,       // input token to swap /sell ie: DAI   
        uint256 inputAmount,     // amount of inout token to sell ie : DAI
        IERC20 outputToken,      // output token to buy ie: WETH
        uint256 minOutputAmount, // minimum output amount expected to receive 
        uint64 deadline         // deadline for when the txn should expire
    )
        public
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
        // @audit -LOW protocol is giving wrong return
        ...
        returns (uint256 inputAmount)
    {
        ...
        // If i want 10 output weth , and my input is DAI
        // send the txn , but the pool get a massive txn that changes the price
        // having slippage protection
        //❌ here is 10 WETH -> DAI
        //✅ here is 10 WETH —> at least 100 DAI
        //✅ how much WETH do I need to give you, ill do a max of 10 —> 100 DAI

        // not having slippage protection
        // here is 10 weth —> gimme the DAI equivalent
        // I want 10 DAI, charge me as much WETH as needed
